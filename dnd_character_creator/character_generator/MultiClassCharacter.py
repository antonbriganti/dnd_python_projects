...
class MultiCharacter:
    ...
    def multi_class(self):
        ...
        if self.usermade:
            ...
            mu_ind = randint(0, len(act_possible)-1)
            mu_class = self.classlist[act_possible[mu_ind][0]]

            mu_ind2 =
            mu_subclass =

            randint(0, len(subclass_list[class_ind])-1)
    ...
    def print_character(self):
            ...
            print('Level: ' + str(self.totallevel))
    ...
    def level_up(self):
        ...
        options = ['level up class', 'multiclass']
        lvl_choice = self.input_loop(options, "What do you want to do? ")

        if lvl_choice == 0:
            class_ind = self.input_loop(self.m_class, "Which class will you level up? ")
            self.level_list[class_ind] += 1

            for i in range(len(self.m_class)):
                print(self.m_class[i], "Level:", self.level_list[i])
            print()

            asi = [4, 8, 12, 16, 19]
            if self.m_class[class_ind]:
                asi = [4, 6, 8, 12, 14, 16, 19]

            if self.level_list[class_ind] in asi:
                # asi allocation
                stats = ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma']
                points = 2
                while points != 0:
                    print('You have ' + str(points) + ' point(s) remaining')
                    choice = self.input_loop(stats, "Which ability would you like to increase? ")
...
char = MultiCharacter(True, True, True, False)
char.char_generator()
char.level_up()
