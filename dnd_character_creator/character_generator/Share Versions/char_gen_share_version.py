...
def multi_class(base_class, abilities, expansion, homebrew):
    ...
    return multi_subclass, multi_mainclass


def print_stats(generator, multi, expansion, homebrew):
    p_subrace, p_race, p_background, p_subclass, p_class, ability_score = generator(expansion, homebrew)
    ...
    if p_subrace != '':
        ...
        print(vowel_check(p_race, True) + p_race)

    print("They are " + vowel_check(p_background, False) + p_background)
    print("Who is " + vowel_check(p_subclass, False) + p_subclass + ' ' + p_class)
    if multi:
        multi_sub, multi_mainclass = multi_class(p_class, ability_score, expansion, homebrew)
        print('And your multi class is...')
        print(vowel_check(multi_sub, True) + multi_sub + ' ' + multi_mainclass)
...
def gen_choose():
    print('1 - Pure RNG Character')
    print('2 - Optimised RNG Character')
    print('3 - Multiclassed Character with Optimised Base')
    print('4 - Pure RNG Multiclassed Character (Experimental)')

    choice = input("Enter your choice: ")

    if choice == '1':
        expansion = setting_choose('expansions')
        homebrew = setting_choose('homebrew')
        print_stats(pure_rng_char, False, expansion, homebrew)

    elif choice == '2':
        expansion = setting_choose('expansions')
        homebrew = setting_choose('homebrew')
        print_stats(optimised_rng_char, False, expansion, homebrew)

    elif choice == '3':
        expansion = setting_choose('expansions')
        homebrew = setting_choose('homebrew')
        print_stats(optimised_rng_char, True, expansion, homebrew)

    elif choice == '4':
        expansion = setting_choose('expansions')
        homebrew = setting_choose('homebrew')
        print_stats(pure_rng_char, True, expansion, homebrew)
...
print("In theory, there's a possible endless loop when you choose multiclassing.")
print("If you find that after selecting multiclass, nothing happens, please reset")
print()

possible_negatives = ['N', 'n', "No", 'no', 'Nope', 'nope', 'fuck you I hope you die in a car accident']
answer = ''
while answer not in possible_negatives:
    gen_choose()
